...
public :
	CIcuECModule()
	{
		...
		if (!pszDir || !*pszDir)
		{
			...
			if( keyFW.Open(HKEY_LOCAL_MACHINE, _T("Software\\SIL"), KEY_READ) == ERROR_SUCCESS )
			{
				...
				if( keyFW.QueryStringValue(_T("Icu48Dir"),lpValue,&nValueSize) == ERROR_SUCCESS )
				{
					USES_CONVERSION;
	...
	DECLARE_REGISTRY_APPID_RESOURCEID(IDR_ICUEC, "{FEF85A1D-E44B-4EF7-8A34-FAA86AFD0EA8}")
};
...
STDAPI DllRegisterServer(void)
{
	...
	// process fails when it tries to replace the entire set of files by unzipping Icu48.zip.
	u_cleanup();  
	return hr;
}
