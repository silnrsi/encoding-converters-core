...
namespace SilEncConverters40
{
    ...
    public class IcuBreakIteratorEncConverter : EncConverter
    {
        ...
        private char[] _achSpace = new char[] { space };
        ...
        protected override unsafe void DoConvert(byte* lpInBuffer, int nInLen, byte* lpOutBuffer, ref int rnOutLen)
        {
            ...
            if (_bForward)
            {
                ...
                if (bySpace.Length == words.Count)
                {
                    ...
                    if (_regexForMandarin.IsMatch(strInput))
                    {
                        strOutput = bySpace
                            ...
                            .Aggregate<char, string>(null, (current, ch) => current + (ch + space));
                    }
                    else
                    {
                        strOutput = strInput;
                ...
                else
                {
                    ...
                    {
                        var word = words[i];
                        if (!String.IsNullOrEmpty(word) && (word != spaceString))
                            strOutput += words[i] + space;
                    }
            ...
            {
                strOutput = strInput.Replace(spaceString, null);
            }
    ...
    public static class IcuBreakIterator
    {
