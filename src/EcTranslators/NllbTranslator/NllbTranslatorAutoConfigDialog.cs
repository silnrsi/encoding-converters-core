...
namespace SilEncConverters40.EcTranslators.NllbTranslator
{
    public partial class NllbTranslatorAutoConfigDialog : AutoConfigDialog
    {
            ...
            )
        {
			...
			var languagesSupported = GetLanguagesSupportedAndInitializeComboBoxes();
		...
		private List<ComboBoxItem> GetLanguagesSupportedAndInitializeComboBoxes()
		{
        ...
        {
            using var dlg = new QueryForEndpointAndApiKey(NllbTranslatorApiKey, NllbTranslatorEndpoint);
            if (dlg.ShowDialog() == DialogResult.OK)
            {
				...
				_nllbTranslator = null;	// so it recreates the client with the new settings

				// in case something changed, reinitialize
				GetLanguagesSupportedAndInitializeComboBoxes();
			}
        }
