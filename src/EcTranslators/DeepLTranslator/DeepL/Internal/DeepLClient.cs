...
namespace DeepL.Internal {
  ...
  internal class DeepLClient : IDisposable {
    ...
    public class AssemblyRedirect {
      public AssemblyRedirect(Version versionDesired, string publicKey) {
        VersionDesired = versionDesired;
        PublicKey = publicKey;
      }
      public Version? VersionDesired { get; set; }
      public string? PublicKey { get; set; }
    }

    public static Dictionary<string, AssemblyRedirect> AssemblyRedirects { get; set; } = new Dictionary<string, AssemblyRedirect>();
    ...
    public static void RedirectAssembly(Dictionary<string, AssemblyRedirect> assemblyRedirects)
    {
      ...
      handler = (sender, args) => {
        ...
        var requestedAssemblyName = requestedAssembly.Name ?? String.Empty;
        if (!assemblyRedirects.TryGetValue(requestedAssemblyName, out var assemblyRedirect))
          return null;
        ...
        requestedAssembly.Version = assemblyRedirect.VersionDesired;
        requestedAssembly.SetPublicKeyToken(new AssemblyName("x, PublicKeyToken=" + assemblyRedirect.PublicKey).GetPublicKeyToken());
        requestedAssembly.CultureInfo = CultureInfo.InvariantCulture;

        //assemblyRedirects.Remove(requestedAssemblyName);
        //if (!assemblyRedirects.Any())
