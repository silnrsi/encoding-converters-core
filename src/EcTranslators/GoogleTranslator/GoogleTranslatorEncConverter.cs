...
namespace SilEncConverters40.EcTranslators.GoogleTranslator
{
    ...
    public class GoogleTranslatorEncConverter : TranslatorConverter
    {
        ...
        public const string EnvVarNameCredentials = "GOOGLE_APPLICATION_CREDENTIALS";   // e.g. @"H:\bright-coyote-381812-bc584cec007f.json";    // downloaded from the APIs & Services => Credentials page in console.cloud.google.com
        ...
        public static string GoogleTranslatorSubscriptionKey
        {
            get
            {
                ...
                var key = Properties.Settings.Default.GoogleTranslatorCredentials;
                ...
                {
                    // see if there's an environment variable instead
                    var overrideKeyEnvVar = Environment.GetEnvironmentVariable(EnvVarNameCredentials);
                    if (!String.IsNullOrEmpty(overrideKeyEnvVar) && File.Exists(overrideKeyEnvVar))
                        return File.ReadAllText(overrideKeyEnvVar);
                }
        ...
        public override bool HasUserOverriddenCredentials
        {
            get
            {
                string pathToEnvVariable;
                return !String.IsNullOrEmpty(Properties.Settings.Default.GoogleTranslatorCredentialsOverride) ||
                      (!String.IsNullOrEmpty((pathToEnvVariable = Environment.GetEnvironmentVariable(EnvVarNameCredentials))) && File.Exists(pathToEnvVariable));
            }
        }
